...
function addPoint(id){
    ...
    if(score[0] >= 10 && !gameOver){
        endGame("BLUE");
        gameOver = true;
    }

    if(score[1] >= 10 && !gameOver){
        endGame("RED");
        gameOver = true;
    }
...
function endGame(winner){
    ...
    if(winner == "RED"){
        document.getElementById("cover").style["backdrop-filter"] = "blur(5px) hue-rotate(90deg)";
        document.getElementById("winnerText").style.color = "rgba(255, 86, 86, 1)";
    } else {
        document.getElementById("cover").style["backdrop-filter"] = "blur(5px) hue-rotate(-90deg)";
    }
}
